...
- **`irreversible-deny` is best-effort defense-in-depth, NOT a security boundary.** Pattern-matching cannot catch all evasions: `sudo -u USER`, `env -i`, exotic wrappers, base64/encoded commands, here-strings, deeply nested shells, or `eval`/`$()` inside the command string. The real boundary is the autonomy ceiling (tier-scaled) + the human checkpoint requirement + Claude Code's own permission system. The hook makes accidental irreversible actions unlikely during normal loop operation; it does not prevent a determined adversary.
- **Cost accrual is advisory in Phase 1.** `cost_so_far_usd` accrues between iterations from `event=="loop_iteration"` rows in `subagent-runs.jsonl` scoped to this loop's `loop_id` and `started_at`. It is data-dependent: if no log rows exist, cost stays 0. It does not halt a within-iteration runaway (Phase-2 live monitor closes that gap).
