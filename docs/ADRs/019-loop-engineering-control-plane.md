...
Phase 1 (the build, shipped as PR #37) is the governed runner. Phase 2 (Supabase `loop_runs`, live mid-flight cost monitor, the ultracode signal) is deferred. Full design: `docs/superpowers/specs/2026-06-20-loop-engineering-design.md`.
...
- **Locked in:** The Stop-hook/`session-state`/`permissionDecision: deny` substrate becomes the shared **governance-hooks base** that ADR-020 (the design-before-code gate) also consumes. The control plane — not foreman — is the home for loop caps.
- **Accepted Phase-1 limits (documented, not defects):** `irreversible-deny` is **best-effort defense-in-depth, not a security boundary** — a pattern-matching Bash hook cannot be made bypass-proof; the real boundary is the autonomy ceiling + human checkpoint + Claude Code's own permission system. Cost enforcement is **advisory** (between-iteration, data-dependent) until the Phase-2 live monitor; `max_recursion_depth` is advisory (`max_iterations` is the hard cap).
